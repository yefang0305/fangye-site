...
from .models import safe_filename


VIDEO_EXTENSIONS = {".3gp", ".avi", ".flv", ".m4v", ".mkv", ".mov", ".mp4", ".mpeg", ".mpg", ".rmvb", ".ts", ".vob", ".webm", ".wmv"}
...
class LocalWhisperASREngine:
    ...
    Transcribes video/audio files using a locally-downloaded Whisper model.
    Supports CUDA acceleration and configurable model sizes.

    Environment variables for model storage:
        HF_HOME, HUGGINGFACE_HUB_CACHE, HF_HUB_CACHE, XDG_CACHE_HOME, CT2_CACHE_DIR
    """
    ...
    def __init__(
        ...
        self.model_dir = Path(model_dir) if model_dir else Path.home() / ".cache" / "whisper_models"
    ...
    def transcribe(self, video_path: str | Path) -> dict:
        ...
        with tempfile.TemporaryDirectory(prefix="benchmark_asr_") as temp_dir:
    ...
    def _load_model(self):
        if self._model is None:
            try:
                ...
                raise LocalASRError("faster-whisper 未安装，无法运行本地 ASR") from exc
