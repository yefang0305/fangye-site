"""CR TubeGet based Douyin resolver/downloader.

This keeps the original workbench route: qjs.exe generates a_bogus,
crck.exe can export Netscape cookies, then the tool downloads the direct
play URL returned by Douyin's detail API.
"""
...
from url_utils import safe_filename, stable_url_hash

logger = logging.getLogger("douyin_cr")
...
class DouyinCRResolver:
    ...
    def resolve(self, url: str) -> dict:
        if not is_douyin_url(url):
            raise DownloadError("CR TubeGet 仅支持抖音链接")
    ...
    def _fetch_aweme_detail(self, session: requests.Session, item_id: str, referer: str, tmp_path: Path) -> dict:
        args = (
...
class DouyinCRVideoDownloader:
    ...
    def download(self, url: str, output_dir: str | Path) -> Path:
        ...
        for candidate in resolved.get("download_urls") or [resolved["direct_url"]]:
            for headers in self._download_headers(resolved.get("referer") or url):
                try:
            ...
            raise DownloadError("CR TubeGet 下载失败: " + "; ".join(errors[-3:]))

        tmp_target.replace(target)
        if target.stat().st_size <= 0:
            raise DownloadError("CR TubeGet 下载到空文件")
        return target

    def _download_candidate(self, candidate: str, tmp_target: Path, headers: dict) -> None:
        with requests.get(candidate, headers=headers, stream=True, timeout=self.timeout, allow_redirects=True) as response:
            response.raise_for_status()
